...
export default function Dashboard() {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* QABs — Critical Alert Quick Action Banners */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {CRITICAL_ALERTS.map(a => {
          const s = GLUCOSE_STATUS[a.type]
          return (
            <div
              key={a.id}
              className="flex items-center gap-4 p-4 rounded-2xl border cursor-pointer hover:shadow-md transition-shadow"
              style={{ backgroundColor: s.bg, borderColor: s.color + '30' }}
            >
              <div className="w-11 h-11 rounded-full flex items-center justify-center text-sm font-bold text-white shrink-0" style={{ backgroundColor: s.color }}>
                {a.patient.split(' ').map(w => w[0]).join('').slice(0, 2)}
              </div>
              <div className="flex-1 min-w-0">
                <p className="m-0 text-sm font-bold text-[#1E293B] truncate">{a.patient}</p>
                <div className="flex items-center gap-1.5 mt-0.5">
                  <span className="font-mono text-sm font-bold" style={{ color: s.color }}>{a.value} mg/dL</span>
                  <StatusBadge status={a.type} map={GLUCOSE_STATUS} />
                </div>
                <p className="m-0 text-xs text-[#64748B] mt-0.5 truncate">{a.time} · {a.doctor}</p>
              </div>
              <button
                onClick={() => navigate('/glucose')}
                className="shrink-0 flex items-center justify-center w-8 h-8 rounded-xl border-0 cursor-pointer text-white"
