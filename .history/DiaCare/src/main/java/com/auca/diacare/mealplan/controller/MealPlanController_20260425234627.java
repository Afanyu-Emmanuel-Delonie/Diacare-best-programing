...
@Tag(name = "Meal Plans", description = "Dietary meal plans assigned by doctors to patients")
...
public class MealPlanController {
    ...
    @Operation(summary = "Create a meal plan for a patient (doctor only)")
    @PostMapping
    public ResponseEntity<MealPlan> create(@Valid @RequestBody MealPlanDTO dto) {
        MealPlan plan = new MealPlan();
        plan.setPatient(patientRepository.findByUser_PublicId(dto.getPatientPublicId())
                .orElseThrow(() -> new RuntimeException("Patient not found")));
        plan.setDoctor(doctorRepository.findByUser_PublicId(dto.getDoctorPublicId())
    ...
    @Operation(summary = "Get my meal plans", description = "Returns plans for the authenticated patient or doctor")
    @GetMapping("/my")
    public ResponseEntity<List<MealPlan>> getMy(Authentication authentication) {
